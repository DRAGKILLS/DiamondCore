...
public class ServerPacketRewriter {
    ...
    public static void rewrite(Player player, BedrockPacket packet) {
        ...
        if(packet instanceof PlayerListPacket) {
            if (((PlayerListPacket) packet).getAction() != PlayerListPacket.Action.ADD) {
                return;
            }
            for (PlayerListPacket.Entry entry : ((PlayerListPacket) packet).getEntries()) {
                entry.setEntityId(convertEntityId(player, entry.getEntityId()));
            }
            return;
        }
        ...
        if(packet instanceof AddPaintingPacket) {
            ((AddPaintingPacket) packet).setRuntimeEntityId(convertEntityId(player, ((AddPaintingPacket) packet).getRuntimeEntityId()));
            ((AddPaintingPacket) packet).setUniqueEntityId(convertEntityId(player, ((AddPaintingPacket) packet).getUniqueEntityId()));
            return;
        }
